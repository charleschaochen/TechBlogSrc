...
分析： 既然要找中位数，很简单就是排序的想法。那么基于字节的桶排序是一个可行的方法 （请见《[桶排序][Link 1] 》）：  
...
整个过程的时间复杂度在O(n)的线性级别上(没有任何循环嵌套)。但主要时间消耗在第一步的第二次内存-磁盘数据交换上，即10G数据分255个文件写回磁盘上。一般而言，如果第二步过后，内存可以容纳下存在中位数的某一个文件的话，直接快排就可以了。关于快排的效率，可以看看我博客中的数据《[基于比较的内部排序总结 ][Link 2] 》。


[Link 1]: http://hxraid.iteye.com/blog/647759
[Link 2]: http://hxraid.iteye.com/blog/646760